  ...
  create: (context) => {
    ...
    const report = imports => {
      ...
      return context.report(
        ...
        fixer => {
          console.log("Replacing", sourceCode.getText().slice(firstImport.start, firstImport.end), "with", sortedImportsText);
          fixer.replaceRange([firstImport.start, firstImport.end], sortedImportsText);
          otherImports.forEach(otherImport => {
            console.log("Replacing", sourceCode.getText().slice(otherImport.start, otherImport.end), "with", "");
