...
const getImportType = importDeclaration => {
  if (!isImport(importDeclaration)) {
    throw new Error(
      `Expected type ImportDeclaration, received type ${
...
const compareImportPaths = (first, second, ignoreCase) => {
  const firstPath = ignoreCase
    ? first.source.value.toLowerCase()
    : first.source.value;
  const secondPath = ignoreCase
    ? second.source.value.toLowerCase()
    : second.source.value;
...
const compareSpecifiers = (first, second, ignoreCase) => {
  const firstName = ignoreCase
    ? first.local.name.toLowerCase()
    : first.local.name;
  const secondName = ignoreCase
    ? second.local.name.toLowerCase()
    : second.local.name;
...
module.exports = {
  ...
  create: context => {
    const configuration = context.options[0] || {},
      importsAtStart =
        configuration.importsAtStart != null
          ? configuration.importsAtStart
          : true,
      sortOrder = configuration.sortOrder || ["type", "named", "unnamed"],
      enforceBlankLine =
        configuration.enforceBlankLine != null
          ? configuration.enforceBlankLine
          : false,
      enforceTrailingLine =
        configuration.enforceTrailingLine != null
          ? configuration.enforceTrailingLine
          : false,
      ignoreCase =
        configuration.ignoreCase != null ? configuration.ignoreCase : true,
    ...
    const reportIncorrectOrder = imports => {
      ...
      allImports.unnamed = imports.filter(
        node => !isTypeImport(node) && !isNamedImport(node),
      );
      const sortedImportsText = sortOrder
        .map(type =>
          allImports[type]
            .sort((first, second) =>
              compareImportPaths(first, second, ignoreCase),
            )
      ...
      return context.report({
        ...
        fix: fixer => {
          return fixer.replaceTextRange(
            [firstImport.start, lastImport.end],
            sortedImportsText,
          );
        },
      });
    ...
    const reportImportAfterNonImport = (node, program) => {
      ...
      const rangeToReplace = [startPoint, endPoint];
    ...
    const reportIncorrectSpecifierOrder = node => {
      ...
      const rangeToReplace = [startPoint, endPoint];
      const replacementText = importSpecifiers
        .slice()
          ...
          const textAfterSpecifier =
            index === importSpecifiers.length - 1
              ...
              : sourceCode
                  .getText()
                  .slice(
                    importSpecifiers[index].end,
                    importSpecifiers[index + 1].start,
                  );
          return (
            acc +
            sourceCode.getText().slice(specifier.start, specifier.end) +
            textAfterSpecifier
          );
        }, "");
      return context.report({
      ...
      Program: program => {
        if (importsAtStart) {
          ...
          if (
            ...
            lastImportIndex !== -1 &&
            firstNonImportIndex < lastImportIndex
          ) {
        ...
        for (let imports of importGroups) {
          ...
          imports.forEach(importDeclaration => {
            if (previousImport != null) {
              ...
              const comparisonByType = compareImportTypes(
                previousImport,
                importDeclaration,
                sortOrder,
              );
              if (comparisonByType === 1) {
                ...
                const comparisonByPath = compareImportPaths(
                  previousImport,
                  importDeclaration,
                  ignoreCase,
                );
                ...
                if (
                  enforceBlankLine &&
                  importDeclaration.loc.start.line <=
                    previousImport.loc.start.line + 1
                ) {
                  reportIncorrectOrder(imports);
                  return;
                }
            ...
            importDeclaration.specifiers
              ...
              .forEach(specifier => {
                if (previousSpecifier != null) {
                  const comparisonBySpecifiers = compareSpecifiers(
                    previousSpecifier,
                    specifier,
                    ignoreCase,
                  );
